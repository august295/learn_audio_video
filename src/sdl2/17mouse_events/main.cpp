...
private:
    ...
    LButtonSprite m_currentButtonSprite;
};

// 纹理
LTexture g_currentTexture;
SDL_Rect g_spriteClips[TOTAL_BUTTONS];
...
bool loadMedia()
{
    ...
    if (g_currentTexture.loadFromFile("resources/button.png") == false)
    {
    ...
    else
    {
        ...
        {
            g_spriteClips[i].x = 0;
            g_spriteClips[i].y = i * BUTTON_HEIGHT;
...
void LButton::handleEvent(SDL_Event* e)
{
    if (e->type == SDL_MOUSEMOTION || e->type == SDL_MOUSEBUTTONDOWN)
    {
        ...
        else
        {
            switch (e->type)
            {
            case SDL_MOUSEMOTION:     m_currentButtonSprite = BUTTON_SPRITE_MOUSE_OVER_MOTION; break;
            case SDL_MOUSEBUTTONDOWN: m_currentButtonSprite = BUTTON_SPRITE_MOUSE_DOWN; break;
            case SDL_MOUSEBUTTONUP:   m_currentButtonSprite = BUTTON_SPRITE_MOUSE_UP; break;
            }
        }
    }
}
...
void LButton::render()
{
    g_currentTexture.render(m_position.x, m_position.y, &g_spriteClips[m_currentButtonSprite]);
}