...
bool loadMedia()
{
    ...
    if (!g_dotTexture.loadFromFile("resources/dot.bmp"))
    {
        SPDLOG_ERROR("Failed to load dot texture!");
        success = false;
    }
...
void Dot::handleEvent(SDL_Event& e)
{
    if (e.type == SDL_KEYDOWN && e.key.repeat == 0)
    {
        ...
        {
        case SDLK_UP:    m_vy -= DOT_VEL; break;
        case SDLK_DOWN:  m_vy += DOT_VEL; break;
        case SDLK_LEFT:  m_vx -= DOT_VEL; break;
        case SDLK_RIGHT: m_vx += DOT_VEL; break;
        default:         break;
        }
    }
...
{
    m_x += m_vx;
    if ((m_x < 0) || m_x + DOT_WIDTH > SCREEN_WIDTH)
    {
        m_x -= m_vx;
    }

    m_y += m_vy;
    if ((m_y < 0) || m_y + DOT_HEIGHT > SCREEN_HEIGHT)
    {
        m_y -= m_vy;
    }
}
